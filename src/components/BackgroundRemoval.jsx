...
import {
  Upload, Download, Layers, Wand2, CheckCircle, RefreshCw,
  Image as ImageIcon, Sparkles, ArrowRight, X, ZoomIn, Eye
} from 'lucide-react';
import { removeBriaBackground } from '../utils/BriaBackgroundService';

const CHECKERBOARD_CSS = `
  repeating-conic-gradient(#1e1530 0% 25%, #2d1e44 0% 50%) 0 0 / 20px 20px
`;
...
export const BackgroundRemoval = () => {
  ...
  const loadFile = (file) => {
    ...
    reader.onload = (e) => {
      ...
      // Draw onto canvas to get image data
      const img = new Image();
      img.onload = () => {
  ...
  const handleRemoveBg = async () => {
    ...
    setProgress(0);
    setError(null);
    setStatusMsg('Sending to Bria AI...');

    try {
      setStatusMsg('Bria AI analyzing subject & background...');
      const resultImg = await removeBriaBackground(imageSrc, (p) => {
        setProgress(p);
        if (p < 30) setStatusMsg('Sending to Bria AI...');
        else if (p < 70) setStatusMsg('Bria AI segmenting subject...');
        else if (p < 90) setStatusMsg('Reconstructing transparent background...');
        else setStatusMsg('Finalizing result...');
      });

      // Draw result on canvas and export as PNG (preserve transparency)
      ...
      setStatusMsg('Background removed successfully!');
    } catch (err) {
  ...
  const handleDownload = () => {
    ...
    const isPng = downloadFormat === 'png';
    link.href = isPng ? resultSrc : (() => {
      // Convert to JPEG if needed (loses transparency)
      const img = new Image();
      img.src = resultSrc;
    ...
    link.download = `${fileName || 'result'}-no-background.${isPng ? 'png' : 'jpg'}`;
  ...
  return (
    <section id="bg-removal" className="py-16 md:py-24 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#171121] border border-[#A855F7]/30 text-xs font-bold text-[#C084FC] mb-5 purple-glow-sm">
            ...
            <span>BRIA AI Background Removal v2</span>
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            <span className="text-gray-300 font-medium">One-Click Instant</span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-extrabold text-white tracking-tight mb-3">
            Remove Background
            <span className="block mt-1 bg-gradient-to-r from-[#C084FC] via-[#A855F7] to-[#9333EA] bg-clip-text text-transparent">
              Instantly & Precisely
            </span>
          </h2>
          <p className="text-base text-gray-400 max-w-xl mx-auto">
            Powered by Farhanbits.dev — automatically isolates your subject with pixel-perfect accuracy and exports a fully transparent PNG with zero quality loss.
          </p>
        </div>
          ...
          <div className="max-w-3xl mx-auto">
            <div
              ...
              className={`relative rounded-[2rem] p-10 sm:p-16 text-center transition-all duration-500 overflow-hidden group ${
                isDragging
                  ? 'scale-[1.02] shadow-[0_0_50px_rgba(52,211,153,0.3)]'
                  : 'hover:scale-[1.01] hover:shadow-[0_20px_60px_rgba(168,85,247,0.15)] shadow-2xl'
              }`}
            >
              {/* Animated glowing border background */}
              <div className={`absolute inset-0 bg-gradient-to-r ${isDragging ? 'from-emerald-400 via-emerald-600 to-emerald-400' : 'from-[#9333EA] via-[#34d399] to-[#9333EA]'} bg-[length:200%_200%] animate-[gradient_4s_ease_infinite] opacity-50`} />
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="relative mb-8">
                  <div className="w-28 h-28 rounded-3xl bg-gradient-to-br from-[#171121] to-[#1a1325] border border-white/5 shadow-[0_0_40px_rgba(52,211,153,0.2)] flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                    <Layers className="w-12 h-12 text-emerald-400 drop-shadow-[0_0_10px_rgba(52,211,153,0.8)] animate-[pulse_3s_ease-in-out_infinite]" />
                  </div>
                ...
                <h3 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 mb-3 tracking-tight">
                  Drag & Drop to Remove Background
                </h3>
                <p className="text-sm text-gray-400 mb-10 max-w-sm leading-relaxed">
                  Upload an image and let our enterprise-grade AI instantly isolate your subject with pixel-perfect precision.
                </p>
                ...
                <label className="cursor-pointer relative group/btn inline-flex items-center gap-2.5 px-10 py-4 rounded-xl bg-white text-[#120F17] font-extrabold text-sm shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] transition-all overflow-hidden hover:scale-[1.02] active:scale-[0.98]">
                  ...
                  <span className="relative z-10">Upload Image File</span>
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="image/*"
                ...
                <div className="mt-10 grid grid-cols-3 gap-4 w-full max-w-md">
                  {[
                    ...
                    { icon: '🖼️', label: 'Lossless Export' },
                    { icon: '✨', label: 'Perfect Edges' },
                  ].map((b) => (
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2 text-xs text-gray-400">
                ...
                {resultSrc && (
                  <>
                    <span>•</span>
                    <span className="text-emerald-400 font-semibold">Background removed ✓</span>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="glass-panel border border-[#A855F7]/25 rounded-2xl overflow-hidden purple-glow-lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-[#A855F7]/10">
                ...
                {/* Result */}
                <div
                  className="relative flex items-center justify-center min-h-[320px] sm:min-h-[440px] p-4"
                  style={{ background: resultSrc ? CHECKERBOARD_CSS : '#0d0a13' }}
                >
                  <span className="absolute top-3 left-3 bg-[#171121]/90 border border-white/10 text-[11px] font-bold text-gray-300 px-2.5 py-1 rounded-lg uppercase tracking-wider z-10">
                    {resultSrc ? 'Background Removed' : 'Result'}
                  </span>

                  {isProcessing ? (
                    <div className="flex flex-col items-center gap-4 max-w-xs text-center">
                      <div className="w-16 h-16 rounded-2xl bg-[#A855F7]/20 border border-[#A855F7]/50 flex items-center justify-center purple-glow">
                        <Layers className="w-8 h-8 text-[#C084FC] animate-pulse" />
                      </div>
                      <div>
                        <p className="text-white font-bold text-sm mb-1">{statusMsg}</p>
                        <p className="text-xs text-gray-400 mb-3">Bria AI is working...</p>
                        <div className="w-48 bg-[#120F17] rounded-full h-2 border border-white/10 overflow-hidden">
                          <div
                            className="bg-gradient-to-r from-[#9333EA] to-[#A855F7] h-full rounded-full transition-all duration-300"
                            style={{ width: `${progress}%` }}
                          />
                        </div>
                        <p className="text-[11px] font-mono text-[#C084FC] mt-1">{progress}%</p>
                      </div>
                    </div>
                  ) : resultSrc ? (
                    <img
                      src={showCompare ? imageSrc : resultSrc}
                      alt="Result"
                      className="max-w-full max-h-[400px] object-contain rounded-lg shadow-2xl"
                    />
                  ) : (
                    <div className="flex flex-col items-center gap-3 text-center opacity-50">
                      ...
                      <p className="text-sm text-gray-500">Your result will appear here</p>
                    </div>
                  )}
              ...
              <div className="bg-[#171121]/95 border-t border-[#A855F7]/20 px-5 py-4 flex items-center justify-between flex-wrap gap-4">
                <div>
                  <p className="text-xs text-gray-400">
                    {resultSrc
                      ? '✓ Background removed. Download your transparent PNG.'
                      : 'Click "Remove Background" to let Bria AI isolate your subject.'}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="relative group/action px-8 py-3 rounded-xl bg-white text-[#120F17] font-extrabold text-sm shadow-[0_0_20px_rgba(52,211,153,0.3)] hover:shadow-[0_0_30px_rgba(52,211,153,0.5)] hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:pointer-events-none flex items-center gap-2 cursor-pointer overflow-hidden border border-emerald-500/20"
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-emerald-400/20 to-transparent -translate-x-full group-hover/action:animate-[shimmer_1.5s_infinite]" />
                    <Layers className="w-4 h-4 relative z-10 text-emerald-600" />
                    <span className="relative z-10">{isProcessing ? 'Removing...' : 'Remove Background'}</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between text-xs text-gray-400 px-1">
              <div className="flex items-center gap-2">
                ...
                <span>Bria AI Background Removal v2 — pixel-perfect subject isolation, fully transparent PNG output.</span>
              </div>
              <span className="text-[#C084FC] mt-1 sm:mt-0">Token: 6a5b626d...e659</span>
            </div>
          </div>
        )}
      </div>
